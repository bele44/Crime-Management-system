...
<html>
<head>
<title>Detective Officer View Accused page</title>
...
<?php
$query = mysqli_query($con,"select * from accused ORDER BY date DESC")
or die(mysqli_error());
$coun = mysqli_num_rows($query);
?>		
										
<a href="DetectiveOfficerViewAcussed.php"><i class="icon-envelope-alt"><font size="4px" color="#a04eb1">Number of Recored(&nbsp;<?php 
$count_item=mysqli_query($con,"select * from accused " ) or die(mysql_error());
$count=mysqli_num_rows($count_item);	
echo"<font color='#da240a'>".($count)."</font>"; ?>)</font></i></a>

	<font size="4px">							
	<?php
	$query1 = mysqli_query($con,"select * from accused ORDER BY date DESC")
	or die(mysqli_error());
	?>

	<?php
	$sql = "select * from accused ORDER BY date DESC";
